...
function setSlide(slide_number) {
  ...
  if(visible_slide){visible_slide.classList.add('displayed');}
}
...
const toggleWhite = (ev) => {
  ...
  for(const element of elements){element.classList.remove("buttonWhite")}
  ev.target.classList.add("buttonWhite")
  setSlide(selected)
}
for (const element of elements) {element.addEventListener('click', toggleWhite)
}
...
window.setInterval(function () {
  ...
  if (selected==0){slidebutton1.classList.add("buttonWhite")}
  if (selected==1){slidebutton2.classList.add("buttonWhite")}
  if (selected==2){slidebutton3.classList.add("buttonWhite")}
  setSlide(selected);
}, 5000);
...
storeSearch.addEventListener('input', ev =>{
  ...
  for(const result of document.querySelectorAll('.hidden')){result.classList.remove('hidden');}
